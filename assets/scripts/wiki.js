...
function createPanel(parent, className, id) {
    const panel = parent.appendChild(document.createElement('div'));
    panel.className = className;
    panel.id = id;
    return panel;
}
function createText(parent, tagName, className, id, innerText) {
    const textObject = {
...
function createImage(parent, source, className, id) {
    const imageObject = {
        area: parent.appendChild(document.createElement('div')),
        object: undefined
    };
...
function setupImageCarousel(imagesPanel, images) {
    let currentIndex = 0;
    // Create buttons
